...
namespace ExcelBrowser.Interop {
    ...
    /// </summary>
    public static class ProcessWindowZ_Extensions {
        ...
        /// <exception cref="InvalidOperationException">Sequence cannot be empty.</exception>
        public static Process TopMost(this IEnumerable<Process> processes) {
            if (processes == null) throw new ArgumentNullException(nameof(processes));
            if (!processes.Any()) throw new InvalidOperationException("Sequence cannot be empty.");

            return processes.OrderByZ().First();
        }
    }
}
